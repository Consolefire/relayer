...
class ReaderCheckpointIndexMonitorTest {
    ...
    private static final ConsumerQueue<CheckpointCompletedEvent> cpEventQueue1
        = new ConsumerQueue<>();
    private static final ConsumerQueue<CheckpointCompletedEvent> cpEventQueue2
        = new ConsumerQueue<>();
    ...
    private ReaderCheckpointIndexMonitor readerCheckpointIndexMonitor;
    ...
    void init() {
        ...
        readerCheckpointIndexMonitor = new ReaderCheckpointIndexMonitor(readerCheckpointService,
            this.checkpointConsumerExecutorService);

        cpEventExecutorService.submit(
            new ReaderCheckpointCompletedConsumer(SRC_ID_1, readerCheckpointIndexMonitor, cpEventQueue1));
        cpEventExecutorService.submit(
            new ReaderCheckpointCompletedConsumer(SRC_ID_2, readerCheckpointIndexMonitor, cpEventQueue2));
    }
    ...
    @Test
    void someTest() {
        readerCheckpointIndexMonitor.initialize(SRC_ID_1);
        readerCheckpointIndexMonitor.initialize(SRC_ID_2);
            ...
            .forEach(i ->
                {
                    producerExecutor.submit(
                        () -> {
                            readerCheckpointIndexMonitor.process(SRC_ID_1, new CheckpointIndex(i, expectedTotal));
                        });
                    producerExecutor.submit(
                        () -> {
                            readerCheckpointIndexMonitor.process(SRC_ID_2, new CheckpointIndex(i, expectedTotal));
                        });
                }
            );
    }
