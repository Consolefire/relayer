...
@Slf4j
public class ReaderCheckpointIndexProcessor {
    ...
    private final ConcurrentConsumerQueue<CheckpointCompletedEvent> concurrentConsumerQueue;

    private volatile CountDownLatch countDownLatch;

//    public ReaderCheckpointIndexProcessor(String sourceIdentifier, ReaderCheckpointService readerCheckpointService) {
...
//    }
    ...
    public ReaderCheckpointIndexProcessor(
        String sourceIdentifier,
        ConcurrentConsumerQueue<CheckpointCompletedEvent> concurrentConsumerQueue) {
        this.sourceIdentifier = sourceIdentifier;
        this.concurrentConsumerQueue = concurrentConsumerQueue;
        this.indexMapReference.set(new ConcurrentHashMap<>());
    }
    ...
    public void processIndex(CheckpointIndex index) {
        ...
        if (completed()) {
            ...
            concurrentConsumerQueue.add(new CheckpointCompletedEvent(sourceIdentifier, true));
        } else {
